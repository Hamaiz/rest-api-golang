...
func (a *AccountDatabase) GetUser(id string) (model.UserSend, error) {
	u := model.UserSend{}
	row := a.conn.QueryRow(context.Background(), "SELECT name, email, unique_name FROM account WHERE id=$1", id)
...
func (a *AccountDatabase) ConfirmEmail(t string) (bool, error) {
	...
	_, err = a.conn.Exec(context.Background(), "UPDATE addition SET token=$1, confirmed=$2", "", true)
...
func (a *AccountDatabase) GetToken(e string) (string, error) {
	...
	var token string
	row = a.conn.QueryRow(context.Background(), "SELECT token FROM addition WHERE account_id=$1", id)
	err = row.Scan(&token)
